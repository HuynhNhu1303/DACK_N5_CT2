...
server.post("/auth/orders", (req, res) => {
  const { shirtId, customerName } = req.body;
  const exist_shirt_id = db.shirts.findIndex(shirt => shirt.id === shirtId);
  ...
  const order_shirt = db.shirts[exist_shirt_id];
  if (order_shirt.available) {
    const new_order = {
